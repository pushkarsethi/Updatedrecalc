...
(function initParticles() {
  ...
  class Particle {
    ...
    reset() {
      ...
      this.col = Math.random() < 0.12
        ? `rgba(184,255,62,${this.a})`
        : `rgba(180,190,220,${this.a})`;
    }
    update() {
...
/* ══════════════════════════════════════════════════════════
   TICKER TAPE — NSE Stocks (Nifty 50 · Sensex · Nifty IT)
   ══════════════════════════════════════════════════════════ */
(function initTicker() {
  const inner = document.getElementById('tickerInner');
  if (!inner) return;

  // Nifty 50 + Sensex stocks + Nifty IT stocks (simulated live prices)
  const stocks = [
    // Nifty 50 / Sensex heavyweights
    ...
    // Indices
    { sym: 'NIFTY 50',   price: 24198.30, chg: +0.42 },
    { sym: 'SENSEX',     price: 79671.58, chg: +0.38 },
    { sym: 'NIFTY IT',   price: 38920.40, chg: +1.24 },
    { sym: 'NIFTY BANK', price: 52134.75, chg: -0.15 },
    // Commodities & rates
    { sym: 'GOLD MCX',   price: 73458,    chg: +0.62 },
    { sym: 'USD/INR',    price: 83.47,    chg: -0.08 },
    { sym: 'REPO RATE',  price: '6.50%',  chg: null  },
  ];

  function buildItems(list) {
    return list.map(s => {
      const priceStr = typeof s.price === 'number'
        ? s.price.toLocaleString('en-IN', { maximumFractionDigits: 2 })
        : s.price;
      const chgHtml = s.chg !== null
        ? `<span class="t-chg ${s.chg < 0 ? 'neg' : 'pos'}">${s.chg >= 0 ? '▲' : '▼'} ${Math.abs(s.chg).toFixed(2)}%</span>`
        : '';
      return `<div class="ticker-item"><span class="t-name">${s.sym}</span><span class="t-val">${priceStr}</span>${chgHtml}</div>`;
    }).join('');
  }

  // Build doubled content for seamless loop
  const html = buildItems(stocks);
  inner.innerHTML = html + html;

  // Simulate subtle price flickers every 4s
  setInterval(() => {
    const items = inner.querySelectorAll('.ticker-item .t-val');
    const chgItems = inner.querySelectorAll('.ticker-item .t-chg');
    const idx = Math.floor(Math.random() * (items.length / 2));
    const delta = (Math.random() - 0.48) * 0.3;
    if (chgItems[idx]) {
      const cur = parseFloat(chgItems[idx].textContent) || 0;
      const newChg = +(cur + delta).toFixed(2);
      chgItems[idx].textContent = (newChg >= 0 ? '▲ ' : '▼ ') + Math.abs(newChg).toFixed(2) + '%';
      chgItems[idx].className = 't-chg ' + (newChg >= 0 ? 'pos' : 'neg');
      // Mirror in duplicate
      if (chgItems[idx + items.length / 2]) {
        chgItems[idx + items.length / 2].textContent = chgItems[idx].textContent;
        chgItems[idx + items.length / 2].className   = chgItems[idx].className;
      }
    }
  }, 4000);
})();
...
function toggleTheme() {
  ...
  el('theme-icon').textContent = document.body.classList.contains('light') ? '☾' : '☀';
}
    ...
    options: {
      ...
      plugins: {
        legend: {
          ...
          labels: { boxWidth: 8, boxHeight: 8, borderRadius: 4, font: { family: 'Space Mono', size: 9 }, color: '#4e5568', padding: 14 },
        },
        tooltip: {
          backgroundColor: 'rgba(8,11,16,0.96)', borderColor: 'rgba(255,255,255,0.07)',
          borderWidth: 1, titleFont: { family: 'Space Mono', size: 10 },
          bodyFont: { family: 'Space Mono', size: 11 }, padding: 12, cornerRadius: 8,
          callbacks: {
            label: c => '  ' + fmtLakh(c.raw),
          },
        },
      },
      scales: {
        x: { grid: { color: 'rgba(255,255,255,0.03)' }, ticks: { font: { family: 'Space Mono', size: 8 }, color: '#4e5568', maxTicksLimit: 10 } },
        y: { grid: { color: 'rgba(255,255,255,0.03)' }, ticks: { font: { family: 'Space Mono', size: 8 }, color: '#4e5568', callback: v => fmtLakh(v) } },
      },
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          backgroundColor: 'rgba(8,11,16,0.96)', borderColor: 'rgba(255,255,255,0.07)',
          borderWidth: 1, titleFont: { family: 'Space Mono', size: 10 },
          bodyFont: { family: 'Space Mono', size: 11 }, padding: 10, cornerRadius: 8,
...
function calcCompound() {
  ...
  compoundChartInst = buildLineChart('compoundChart', labels, [
    ...
    { label: 'Value', data: aArr, borderColor: '#f5c842',
      backgroundColor: (ctx) => { const g = ctx.chart.ctx.createLinearGradient(0,0,0,180); g.addColorStop(0,'rgba(245,200,66,0.16)'); g.addColorStop(1,'rgba(245,200,66,0)'); return g; },
...
function calcSIP() {
  ...
  sipChartInst = buildLineChart('sipChart', labels, [
    ...
    { label: 'Corpus', data: corArr, borderColor: '#00e5a0',
      backgroundColor: (ctx) => { const g = ctx.chart.ctx.createLinearGradient(0,0,0,180); g.addColorStop(0,'rgba(0,229,160,0.15)'); g.addColorStop(1,'rgba(0,229,160,0)'); return g; },
      fill: true, tension: 0.4 },
  ], sipChartInst);
...
function calcGoal() {
  ...
  goalChartInst = buildLineChart('goalChart', labels, [
    ...
    { label: 'Corpus', data: corArr, borderColor: '#b97fff',
      backgroundColor: (ctx) => { const g = ctx.chart.ctx.createLinearGradient(0,0,0,180); g.addColorStop(0,'rgba(185,127,255,0.14)'); g.addColorStop(1,'rgba(185,127,255,0)'); return g; },
      fill: true, tension: 0.4 },
    { label: 'Goal', data: tgtArr, borderColor: 'rgba(184,255,62,0.5)', borderDash: [6,3], pointRadius: 0, fill: false, tension: 0 },
  ], goalChartInst);
  ...
  if (sbody) {
    ...
    rates.forEach(rr => {
      ...
      sbody.innerHTML += `<tr ${isCurrent ? 'style="background:rgba(184,255,62,0.06)"' : ''}>
